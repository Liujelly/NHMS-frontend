  ...
  <div>
      <div style="margin: bottom 30px;">
        <el-breadcrumb separator="/">
          ...
          <el-breadcrumb-item>物资归还</el-breadcrumb-item>
        </el-breadcrumb>
      ...
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="outboundNumber" label="借用编号" width="120">
          </el-table-column>
          ...
          <el-table-column prop="warehousingTime" label="借用时间" >
          </el-table-column>
          ...
          <el-table-column prop="inOperator" label="借用人" >
          </el-table-column>
          <el-table-column label="操作" >
              <template slot-scope="scope">
                <el-button type="success" size="small" @click="handleIWare(scope.row)">归还<i class="el-icon-s-fold" ></i></el-button>
                <el-popconfirm
                class="ml-5"
                  confirm-button-text='确定'
                  cancel-button-text='不用了'
    ...
    :total="total">
      ...
      <el-dialog title="物资信息" :visible.sync="dialogFormVisible" width="30%">
        ...
        <el-input v-model="form.inOperator" autocomplete="off" :disabled="true"></el-input>
        </el-form-item>
  ...
  methods:{
    load () {
      if(this.inboundNumber=="" ){
          request.get( "/Return/page?pageNum="+this.pageNum+"&pageSize="+this.pageSize+"&materialName="+this.materialName+"&inOperator="+this.inOperator).then(res=>{console.log(res)
      this.tableData=res.date
      this.total=res.total
      })
      }
      else{
        request.get( "/Return/page1?pageNum="+this.pageNum+"&pageSize="+this.pageSize+"&inboundNumber="+this.inboundNumber).then(res=>{console.log(res)
      this.tableData=res.date
      this.total=res.total
      })
      }
...
this.form.warehousingQuantity=0
    ...
    inWarehouse(){
      ...
      request.post("/Return/InWarehouse",this.form).then(res=>{
        if(res){
...
console.log(val)
  this.multipleSelection=val
    ...
    dalBatch(){
      ...
      request.post("/Return/batch",mIds).then(res=>{
      if(res!=null){
  ...
  //导出
exp(){
  window.open("http://124.223.217.249:8081/Return/export")
},
