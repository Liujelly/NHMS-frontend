<template>
  <div>
      ...
      <el-table :data="tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="outboundNumber" label="出库编号" width="120">
          </el-table-column>
...
import request from '@/utils/request'
  ...
  methods:{
    load () {
      if(this.inboundNumber=="" && this.warehousingTime==""){
          request.get( "/OutRecord/page?pageNum="+this.pageNum+"&pageSize="+this.pageSize+"&materialName="+this.materialName+"&inOperator="+this.inOperator).then(res=>{console.log(res)
      this.tableData=res.date
      this.total=res.total
      })
      }if(this.inboundNumber!=""){
        request.get( "/OutRecord/page1?pageNum="+this.pageNum+"&pageSize="+this.pageSize+"&inboundNumber="+this.inboundNumber).then(res=>{console.log(res)
      this.tableData=res.date
      this.total=res.total
      })
      }
      if(this.inboundNumber==""&& this.warehousingTime!=""){
        request.get( "/OutRecord/page2?pageNum="+this.pageNum+"&pageSize="+this.pageSize+"&warehousingTime="+this.warehousingTime).then(res=>{console.log(res)
      this.tableData=res.date
      this.total=res.total
      })
      }
  ...
  //导出
exp(){
  window.open("http://124.223.217.249:8081/OutRecord/export")
},
